    ...
    <div class="card-body p-4">
        ...
        <form action="{{ route('users.store') }}" method="POST">
                        ...
                        <select name="role_id" class="form-select input-dark rounded-3" required>
                            ...
                            @if(isset($roles) && count($roles) > 0)
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}">{{ $role->nama_role ?? $role->name }}</option>
                                @endforeach
                            @else
            ...
            <div class="d-flex justify-content-end gap-2">
                <a href="{{ route('users.index') }}" class="btn btn-outline-secondary px-4 rounded-3 text-white border-secondary">
                    &larr; Kembali
                </a>
