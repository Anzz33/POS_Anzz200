...
<div class="card users-card-main border-0 shadow-lg overflow-hidden">
    <div class="card-body p-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom border-secondary border-opacity-25">
            ...
            <div>
                <a href="{{ route('users.create') }}" class="btn btn-warning text-dark fw-bold px-4 py-2 rounded-3 shadow-sm d-inline-flex align-items-center gap-2">
                    + Tambah User
                </a>
            </div>
        </div>
                        ...
                        @forelse($users as $index => $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    @endphp
                                    <span class="badge {{ $isAdmin ? 'badge-role-admin' : 'badge-role-kasir' }} px-3 py-1 rounded-pill fw-semibold text-uppercase" style="font-size: 0.7rem;">
                                        {{ $roleName }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center gap-1">
                                        <a href="{{ route('users.edit', $item->id) }}" class="btn btn-sm btn-warning text-dark rounded-2 px-2 py-1 fw-semibold" style="font-size: 0.75rem;">
                                            Edit
                                        </a>
                                        <form action="{{ route('users.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus user ini?')">
                                            @csrf
                                            @method('DELETE')
